...
namespace NGUInjector
{
    public static class Extensions
    {
        ...
        public static BoostsNeeded GetNeededBoosts(this Equipment eq)
        {
            ...
            if (eq.capAttack != 0.0)
                n.Power += Math.Max(0m, CalcCap(eq.capAttack, eq.level) - (decimal)eq.curAttack);

            if (eq.capDefense != 0.0)
                n.Toughness += Math.Max(0m, CalcCap(eq.capDefense, eq.level) - (decimal)eq.curDefense);

            if (Settings.SpecialBoostBlacklist.Contains(eq.id))
                return n;

            if (eq.spec1Type != specType.None)
                n.Special += Math.Max(0m, CalcCap(eq.spec1Cap, eq.level) - (decimal)eq.spec1Cur);

            if (eq.spec2Type != specType.None)
                n.Special += Math.Max(0m, CalcCap(eq.spec2Cap, eq.level) - (decimal)eq.spec2Cur);

            if (eq.spec3Type != specType.None)
                n.Special += Math.Max(0m, CalcCap(eq.spec3Cap, eq.level) - (decimal)eq.spec3Cur);
        ...
        {
            if (!Settings.GlobalEnabled && (Main.Autopilot == null || !Main.Autopilot.CanExecuteSafe))
                return;
            ...
            try
            {
                ...
                if (Settings.ManageNGUDiff || Main.AutopilotWants(x => x.ManageAllocations))
                    allocation.SwapNGUDiff();
                // NGU targets are finite stopping points unless the game's own
                // auto-advance flag is enabled.  Full automation must never leave
                // an unlocked NGU silently idle after reaching a target.
                if (Main.AutopilotWants(x => x.ManageAllocations) &&
                    Main.Character.buttons.ngu.interactable &&
                    !Main.Character.NGU.autoAdvance)
                {
                    Main.Character.NGU.autoAdvance = true;
                    Main.Character.NGUController.refreshMenu();
                    Main.LogAllocation("Enabled and verified NGU auto-advance");
                }
                if (Settings.ManageGear)
                    allocation.EquipGear();
                if (Settings.ManageEnergy || Main.AutopilotWants(x => x.ManageAllocations))
                    allocation.AllocateEnergy();
                if (Settings.ManageMagic || Main.AutopilotWants(x => x.ManageAllocations))
